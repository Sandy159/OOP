...
Tritset::Proxy_trit::Proxy_trit(int index, const vector<unsigned int>& arr,
    const Tritset& tritset)
    : index_(index), arr_(const_cast<vector<unsigned int>&>(arr)), tritset_(const_cast<Tritset&>(tritset)) {
    value = get_value();
}
...
Tritset::Iterator::Iterator(const int index, const Tritset& tritset) : index_(index), tritset_(tritset) {}

Tritset::Iterator Tritset::Iterator::operator++() {
    index_++;
    return Iterator(index_, tritset_);
}

bool Tritset::Iterator::operator==(Iterator& iter) {
    if (this->index_ == iter.index_ && &((*this).tritset_) == &iter.tritset_)
        return true;
    return false;
}
...
Tritset::Proxy_trit Tritset::Iterator::operator*() {
    return tritset_[index_];
}
...
int Tritset::get_size_in_trit() const { return current_size; }

Tritset::Proxy_trit Tritset::operator[](int index) const {
    Proxy_trit p_trit(index, arr, *this);
    return p_trit;
}

Tritset::Iterator Tritset::begin() const {
    return Iterator(0, *this);
}

Tritset::Iterator Tritset::end() const {
    return Iterator(this->current_size, *this);
}

int Tritset::get_last_set_int_index() const {
...
int Tritset::get_last_set_trit_index() const {
    int trit_index =
...
size_t Tritset::length() const { return get_last_set_trit_index() + 1; }